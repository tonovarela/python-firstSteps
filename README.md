# 🐍 Primeros Pasos con Python

Repositorio con ejercicios básicos para aprender los fundamentos de Python.

## 📚 Contenido

### Primitivas

#### 01 - Hola Mundo
**Archivo:** `primitivas/01-hola_mundo.py`

Primer programa en Python mostrando un mensaje en consola.

```bash
python3 primitivas/01-hola_mundo.py
```

#### 07 - Índices de Strings
**Archivo:** `primitivas/07-string-indexes.py`

Aprende a manipular cadenas de texto usando índices:
- Acceso a caracteres individuales
- Slicing (cortar cadenas)
- Invertir cadenas con `[::-1]`

```bash
python3 primitivas/07-string-indexes.py
```

#### 08 - Inmutabilidad
**Archivo:** `primitivas/08-inmutable.py`

Demuestra que los strings en Python son inmutables (no se pueden modificar).

```bash
python3 primitivas/08-inmutable.py
```

#### 09 - Conversiones de Tipo
**Archivo:** `primitivas/09-conversions.py`

Conversión entre diferentes tipos de datos:
- `int()` - Convertir a entero
- `str()` - Convertir a string
- `float()` - Convertir a decimal

```bash
python3 primitivas/09-conversions.py
```

#### 10 - Operaciones Matemáticas
**Archivo:** `primitivas/10-math_operations.py`

Operaciones matemáticas básicas:
- Suma, resta, multiplicación, división
- Potencia (`**`)
- División entera (`//`)
- Módulo (`%`)
- Funciones: `round()`, `abs()`

```bash
python3 primitivas/10-math_operations.py
```

#### 11 - Funciones Incorporadas
**Archivo:** `primitivas/11-incorporated_functions.py`

Funciones y métodos nativos de Python:
- `type()` - Obtener el tipo de dato
- `.upper()` - Convertir a mayúsculas
- `.replace()` - Reemplazar texto

```bash
python3 primitivas/11-incorporated_functions.py
```

#### 12 - Input
**Archivo:** `primitivas/12-input.py`

Captura de datos del usuario con la función `input()`.

```bash
python3 primitivas/12-input.py
```

#### 13 - Proyecto Final
**Archivo:** `primitivas/13-last.py`

Proyecto que combina todos los conceptos aprendidos:
- Captura de datos con `input()`
- Uso de diccionarios
- F-strings para formateo
- Operaciones matemáticas
- Manipulación de strings

```bash
python3 primitivas/13-last.py
```

### Condicionales

#### 01 - If Condicional
**Archivo:** `condicionales/01-if_condicional.py`

Introducción a las estructuras condicionales:
- Declaración `if`
- Cláusula `else`
- Evaluación de booleanos

```bash
python3 condicionales/01-if_condicional.py
```

#### 02 - Elif Condicional
**Archivo:** `condicionales/02-elif_conditional.py`

Condiciones múltiples con `elif`:
- Múltiples condiciones
- Flujo de control con `elif`
- Uso de `pass` para bloques vacíos

```bash
python3 condicionales/02-elif_conditional.py
```

#### 03 - Operador Ternario
**Archivo:** `condicionales/03-ternary_operator.py`

Condicionales en una sola línea.

```bash
python3 condicionales/03-ternary_operator.py
```

#### 04 - Truthy y Falsey
**Archivo:** `condicionales/04-truthy-falsey.py`

Valores que se evalúan como verdadero o falso.

```bash
python3 condicionales/04-truthy-falsey.py
```

#### 05 - None
**Archivo:** `condicionales/05-none.py`

Manejo del valor `None` en Python.

```bash
python3 condicionales/05-none.py
```

#### 06 - Operadores Lógicos
**Archivo:** `condicionales/06-logical_operators.py`

Uso de `and`, `or`, `not`.

```bash
python3 condicionales/06-logical_operators.py
```

#### 07 - Operadores de Comparación
**Archivo:** `condicionales/07-comparation_operatos.py`

Comparaciones: `==`, `!=`, `>`, `<`, `>=`, `<=`.

```bash
python3 condicionales/07-comparation_operatos.py
```

#### 08 - Operadores de Membresía
**Archivo:** `condicionales/08-membership_operators.py`

Uso de `in` y `not in`.

```bash
python3 condicionales/08-membership_operators.py
```

#### 09 - Is Equal
**Archivo:** `condicionales/09-is_equal.py`

Diferencia entre `is` y `==`.

```bash
python3 condicionales/09-is_equal.py
```

#### 10 - Short Circuiting
**Archivo:** `condicionales/10-short_circuiting.py`

Evaluación de cortocircuito en expresiones lógicas.

```bash
python3 condicionales/10-short_circuiting.py
```

#### 11 - Control Flow
**Archivo:** `condicionales/11-control_flow.py`

Flujo de control avanzado.

```bash
python3 condicionales/11-control_flow.py
```

#### 12 - Proyecto Final
**Archivo:** `condicionales/12-proyecto_final.py`

Proyecto integrador de condicionales.

```bash
python3 condicionales/12-proyecto_final.py
```

### Estructuras (Listas)

#### 02 - List Slicing
**Archivo:** `estructuras/02-list-slicing.py`

Cortar y manipular listas con índices.

```bash
python3 estructuras/02-list-slicing.py
```

#### 03 - Métodos para Agregar
**Archivo:** `estructuras/03-methods_adding.py`

Métodos: `append()`, `extend()`, `insert()`.

```bash
python3 estructuras/03-methods_adding.py
```

#### 04 - Métodos para Remover
**Archivo:** `estructuras/04-list_methods_remove.py`

Métodos: `remove()`, `pop()`, `clear()`.

```bash
python3 estructuras/04-list_methods_remove.py
```

#### 05 - Métodos de Búsqueda
**Archivo:** `estructuras/05-list-methods_search.py`

Métodos: `index()`, `count()`, `in`.

```bash
python3 estructuras/05-list-methods_search.py
```

#### 06 - Métodos de Ordenamiento
**Archivo:** `estructuras/06-list-methods_sort.py`

Métodos: `sort()`, `sorted()`, `reverse()`.

```bash
python3 estructuras/06-list-methods_sort.py
```

#### 07 - Otros Métodos
**Archivo:** `estructuras/07-list_others.py`

Métodos adicionales de listas.

```bash
python3 estructuras/07-list_others.py
```

#### 08 - Matrices
**Archivo:** `estructuras/08-matrix.py`

Listas bidimensionales (matrices).

```bash
python3 estructuras/08-matrix.py
```

#### 09 - List Unpacking
**Archivo:** `estructuras/09-list_unpacking.py`

Desempaquetado de listas.

```bash
python3 estructuras/09-list_unpacking.py
```

#### 10 - Proyecto Final
**Archivo:** `estructuras/10-proyecto_final.py`

Carrito de compras con todas las operaciones de listas.

```bash
python3 estructuras/10-proyecto_final.py
```

### Diccionarios

#### 01 - Diccionarios
**Archivo:** `diccionarios/01-diccionaries.py`

Introducción a los diccionarios (key-value).

```bash
python3 diccionarios/01-diccionaries.py
```

#### 02 - Keys
**Archivo:** `diccionarios/02-keys.py`

Trabajo con llaves de diccionarios.

```bash
python3 diccionarios/02-keys.py
```

#### 03 - Métodos de Diccionarios
**Archivo:** `diccionarios/03-dictionaries_method.py`

Métodos: `keys()`, `values()`, `items()`.

```bash
python3 diccionarios/03-dictionaries_method.py
```

#### 04 - Agregar y Remover
**Archivo:** `diccionarios/04-dictionaries_methods_add_remove.py`

Métodos: `pop()`, `popitem()`, `update()`.

```bash
python3 diccionarios/04-dictionaries_methods_add_remove.py
```

#### Proyecto Final
**Archivo:** `diccionarios/proyecto-final.py`

Sistema de gestión de estudiantes con diccionarios y bucles `for`:
- Agregar estudiantes
- Calcular promedios individuales
- Calcular promedio general
- Iterar con `for ... in`

```bash
python3 diccionarios/proyecto-final.py
```

### Bucles (Loops)

#### 01 - For Loop
**Archivo:** `loop/01-for_loop.py`

Introducción al bucle `for` en Python.

```bash
python3 loop/01-for_loop.py
```

#### 02 - Iterables
**Archivo:** `loop/02-iterable.py`

Qué es un iterable y cómo recorrerlo.

```bash
python3 loop/02-iterable.py
```

#### 03 - Range
**Archivo:** `loop/03-range.py`

Uso de la función `range()` para generar secuencias.

```bash
python3 loop/03-range.py
```

#### 04 - Enumerate
**Archivo:** `loop/04-enumerate.py`

Uso de `enumerate()` para obtener índice y valor en iteraciones.

```bash
python3 loop/04-enumerate.py
```

### Tuplas y Sets

#### 01 - Tuplas
**Archivo:** `tuples-sets/01-tuple.py`

Introducción a tuplas: colecciones inmutables.

```bash
python3 tuples-sets/01-tuple.py
```

#### 02 - Sets
**Archivo:** `tuples-sets/02-set.py`

Conjuntos en Python: colecciones sin elementos duplicados.

```bash
python3 tuples-sets/02-set.py
```

#### 03 - Conversión entre Tipos
**Archivo:** `tuples-sets/03-converts.py`

Convertir entre listas, tuplas y sets.

```bash
python3 tuples-sets/03-converts.py
```

#### Métodos de Sets
**Archivos:**
- `tuples-sets/methods/01-sets_methods.py`
- `tuples-sets/methods/02-methods_part2.py`
- `tuples-sets/methods/03-sets_methods_part3.py`

Aprende los métodos más usados de los conjuntos en Python.

```bash
python3 tuples-sets/methods/01-sets_methods.py
python3 tuples-sets/methods/02-methods_part2.py
python3 tuples-sets/methods/03-sets_methods_part3.py
```

#### Ejercicio Final
**Archivo:** `tuples-sets/exercise.py`

Ejercicios prácticos combinando tuplas y sets.

```bash
python3 tuples-sets/exercise.py
```

### Programación Orientada a Objetos (POO)

#### 01 - Clase Básica
**Archivo:** `poo/01-class.py`

Introducción a clases y objetos en Python:
- Definición de una clase
- Atributos y métodos
- Crear instancias (objetos)

```bash
python3 poo/01-class.py
```

## 📝 Conceptos Aprendidos

- ✅ Sintaxis básica de Python
- ✅ Variables y tipos de datos
- ✅ Strings y manipulación de texto
- ✅ Operaciones matemáticas
- ✅ Funciones incorporadas
- ✅ Input del usuario
- ✅ Diccionarios
- ✅ F-strings
- ✅ Condicionales (if, elif, else)
- ✅ Booleanos y evaluación de condiciones
- ✅ Operadores lógicos y de comparación
- ✅ Listas y sus métodos
- ✅ Iteración con `for ... in`
- ✅ List slicing y unpacking
- ✅ Diccionarios y métodos
- ✅ Tuplas y sets
- ✅ Función `range()` y `enumerate()`
- ✅ Fundamentos de Programación Orientada a Objetos (POO)

## 🚀 Ejecutar los Archivos

Asegúrate de tener Python 3 instalado:

```bash
python3 --version
```

Para ejecutar cualquier archivo:

```bash
python3 carpeta/nombre-del-archivo.py
```

## 🛠️ Requisitos

- Python 3.x

## 📖 Recursos Adicionales

- [Documentación oficial de Python](https://docs.python.org/es/3/)
- [Tutorial de Python en español](https://docs.python.org/es/3/tutorial/)

---

💡 **Tip:** Experimenta modificando los archivos para practicar y entender mejor cada concepto.